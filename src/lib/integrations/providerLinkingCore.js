import { decimalStringToMinorUnits } from './providerMoney.js'
import { stripeChargeMinorUnitExponent } from '../providerAdapters/payments/stripeCurrencyMinorUnits.js'
...
function invoiceMatchesMoney(invoice, money) {
  ...
  try {
    const minor = decimalStringToMinorUnits({
      amount: trimTrailingZeroes(invoice.amount),
      exponent: money.exponent,
    })
    return minor.toString() === money.amountMinor
  } catch {
    return false
  }
}
...
export function deriveStripeInvoiceLinkDecision({
  ...
  // Amount+currency alone is never an automatic merge. Even one candidate is
  // merely a proposal requiring founder confirmation.
