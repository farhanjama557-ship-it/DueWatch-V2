...
// A genuinely different file/mapping naturally hashes to a different key.
export async function buildImportIdempotencyKey(userId, requestRows) {
  if (!userId) throw new Error('buildImportIdempotencyKey: userId is required')
  const hash = await sha256Hex(JSON.stringify(requestRows))
  return `${userId}:${hash}`
}
